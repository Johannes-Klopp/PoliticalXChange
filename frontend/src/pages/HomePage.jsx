...
import { getCandidates, subscribeNewsletter } from '../services/api';
...
export default function HomePage() {
  ...
  const handleNewsletterSubmit = async (e) => {
    ...
    try {
      ...
      setNewsletterStatus({
        type: 'error',
        message: err.response?.data?.error || 'Fehler beim Anmelden'
      });
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-primary-50 via-white to-gray-50">
      ...
      <main className="container mx-auto px-4 py-16 max-w-7xl">
        ...
        {/* Newsletter Section */}
        <section className="mt-16 bg-gradient-to-r from-primary-600 via-primary-700 to-primary-800 rounded-2xl shadow-2xl p-8 md:p-12 text-white" aria-labelledby="newsletter-heading">
          <div className="max-w-2xl mx-auto text-center">
            ...
            <h2 id="newsletter-heading" className="text-3xl font-extrabold mb-3">
              Bleiben Sie informiert
            </h2>
            <p className="text-primary-100 mb-8 text-lg">
              Erhalten Sie Updates zur Wahl und zukünftigen Veranstaltungen
            </p>

            {newsletterStatus && (
              <div
                className={`mb-6 px-6 py-4 rounded-xl shadow-lg ${
                  newsletterStatus.type === 'success'
                    ? 'bg-green-500/20 border border-green-300/50 text-white'
                    : 'bg-red-500/20 border border-red-300/50 text-white'
                }`}
                role="alert"
              >
                <div className="flex items-center justify-center gap-3">
                  ...
                  <span className="font-medium">{newsletterStatus.message}</span>
                </div>
              </div>
            )}
            ...
            <form onSubmit={handleNewsletterSubmit} className="flex flex-col sm:flex-row gap-3 max-w-xl mx-auto">
              ...
              <button
                ...
                className="group bg-white hover:bg-gray-100 text-primary-700 font-semibold px-8 py-4 rounded-xl transition-all duration-300 shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {newsletterLoading ? (
                  <>
